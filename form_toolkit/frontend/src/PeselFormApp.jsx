...
/** String "TRUE"/"FALSE" → boolean dönüşümü */
...
export default function PeselFormApp({ onBack }) {
  ...
  /* ——— Otomatik doldurma: ilk yükleme varsayılanları ——— */
  useEffect(() => {
    setData(prev => {
      const next = { ...prev };
      let changed = false;
      ...
      // Yasal dayanak varsayılanı (kısa ve sade)
      if (!next.s7_legal_basis) {
        next.s7_legal_basis = "KARTA POBYTU - ZEZWOLENIE NA POBYT";
        changed = true;
      }
      // İmza tarihi varsayılanı (bugün)
      if (!next.s8_date_day) {
        next.s8_date_day = String(today.getDate()).padStart(2, "0");
        changed = true;
      }
      if (!next.s8_date_month) {
        next.s8_date_month = String(today.getMonth() + 1).padStart(2, "0");
        changed = true;
      }
      if (!next.s8_date_year) {
        next.s8_date_year = String(today.getFullYear());
        changed = true;
      }
      // Bildirim varsayılanı (kağıt)
      if (!next.s6_notify_paper && !next.s6_notify_electronic) {
        next.s6_notify_paper = true;
        changed = true;
      }
  ...
  /* ——— Otomatik doldurma: türetilmiş alanlar ——— */
  useEffect(() => {
    setData(prev => {
      const next = { ...prev };
      let changed = false;

      // Eş bilgisi varsa → medeni durum "evli" işaretle
      ...
      if (hasSpouseData && noMaritalStatus) {
        next.s4_status_married = true;
        changed = true;
      }

      // Evli ise → evlilik olayını işaretle
      ...
      if (next.s4_status_married && noEventSelected) {
        next.s5_event_marriage = true;
        changed = true;
      }

      // ePUAP adresi varsa → elektronik bildirimi işaretle
      ...
      // İmza şehri boşsa → başvuran adresinden al
      if (!next.s8_city && next.s1_city) {
        next.s8_city = next.s1_city;
        changed = true;
      }

      // Vatandaşlık metni varsa → "diğer" checkbox'ını işaretle
      if (next.s2_citizenship_other_text && !next.s2_citizenship_polish
          && !next.s2_citizenship_stateless && !next.s2_citizenship_other) {
        next.s2_citizenship_other = true;
        changed = true;
      }

      // Başvuran isim/soyad → kişisel bilgilere kopyala (henüz boşsa)
      if (next.s1_name && !next.s2_first_name) {
        next.s2_first_name = next.s1_name;
        changed = true;
      }
      if (next.s1_surname && !next.s2_surname) {
        next.s2_surname = next.s1_surname;
        changed = true;
      }
  ...
  const handleGeneratePDF = useCallback(async () => {
    setPdfLoading(true);
    try {
      await generatePdf(buildExport(), "pesel");
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-950">
      ...
      <header className="flex-shrink-0 h-12 glass border-b border-white/[0.06] flex items-center px-4 gap-3 z-30">
        {onBack && (
          <button onClick={onBack}
            className="text-zinc-500 hover:text-zinc-200 transition-colors text-sm mr-1"
            title="Ana Sayfa">
            &#8592;
          </button>
        )}
      ...
      <main className="flex flex-1 min-h-0">
        ...
        <div className="w-1/2 flex flex-col min-h-0 bg-zinc-900/50 overflow-hidden">
          <PdfPreview data={data} familyData={null} formType="pesel" />
        </div>
      </main>
